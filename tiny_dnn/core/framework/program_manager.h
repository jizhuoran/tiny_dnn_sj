 ...
 */
       nn_info("Compiling the kernel ...");
        ...
        if (build_status != CLCudaAPI::BuildStatus::kSuccess) {
            ...
            __android_log_print(ANDROID_LOG_VERBOSE, "Mytag", "Compiler error(s)/warning(s) found: %s", message.c_str());

            nn_warn("Compiler error(s)/warning(s) found: " +
                    to_string(message.c_str()));
            return;
        }
        __android_log_print(ANDROID_LOG_VERBOSE, "Mytag", "Compiling the kernel ... OK");

 ...
 public:
  ...
  void compute(core::OpKernelContext &context) override {
    ...
    for (size_t i = 0; i < in_data.size(); ++i) {
      ...
      //__android_log_print(ANDROID_LOG_VERBOSE, "My_tag", "The data size is %u, %u, %u, %u", in_data[i].size(), W[0].size(),bias[0].size(), out_data[i].size());
      
      
      //__android_log_print(ANDROID_LOG_VERBOSE, "My_tag", "Time for this 22222222 is !!!!!!! :%lf", (now_ms()- start2));
      ...
      /*    
      __android_log_print(ANDROID_LOG_VERBOSE, "Mytag", "This is a new layer");
      __android_log_print(ANDROID_LOG_VERBOSE, "Mytag", "This is a new layer");
      __android_log_print(ANDROID_LOG_VERBOSE, "Mytag", "This is a new layer");
      __android_log_print(ANDROID_LOG_VERBOSE, "Mytag", "This is a new layer");
      __android_log_print(ANDROID_LOG_VERBOSE, "Mytag", "This is a new layer");
...
*/
      __android_log_print(ANDROID_LOG_VERBOSE, "My_tag", "This is new layer \n\n\n\n\n\n\n");

      //kernel_->Tune(output_ptr, NULL, weights_ptr, NULL, bias_ptr, NULL, input_ptr, NULL, batch_size);
      //double start = now_ms();
      kernel_->Forward(input_ptr, weights_ptr, bias_ptr, output_ptr, batch_size);
      //queue.Finish();
      //__android_log_print(ANDROID_LOG_VERBOSE, "My_tag", "Time for this layer in the inside is !!!!!!! :%lf", (now_ms()- start));
      ...
      double out_start = now_ms();
      std::vector<float_t> out(out_data[i].size(), 0);
      //__android_log_print(ANDROID_LOG_VERBOSE, "My_tag", "Time for (std::vector<float_t> out(out_data[i].size(), 0);) is :%lf", (now_ms()- out_start));
      
      
      
      double read_start = now_ms();
      dev_out.Read(queue, out_data[i].size(), out);
      //__android_log_print(ANDROID_LOG_VERBOSE, "My_tag", "Time for this part 11111 is !!!!!!! :%lf", (now_ms()- read_start));
      /*
      // FOR DEBUG ONLY
 ...
 public:
  ...
  void compute(core::OpKernelContext &context) override {
    ...
    for (size_t i = 0; i < prev_out.size(); ++i) {
      ...
      auto dev_prev_out = CLCudaAPI::Buffer<float_t>(ctx, queue, prev_out[i].begin(),
                                               prev_out[i].end());
      ...
      auto dev_dW =
        CLCudaAPI::Buffer<float_t>(ctx, queue, dW[i].begin(), dW[i].end());

      auto dev_db =
        CLCudaAPI::Buffer<float_t>(ctx, queue, db[i].begin(), db[i].end());

      auto dev_prev_delta =
        CLCudaAPI::Buffer<float_t>(ctx, queue, prev_delta[i].begin(), prev_delta[i].end());

      auto dev_curr_delta =
        CLCudaAPI::Buffer<float_t>(ctx, queue, curr_delta[i].begin(), curr_delta[i].end());
      ...
      float_t *prev_out_ptr   = mutable_double_cast(dev_prev_out());
      float_t *w_ptr = mutable_double_cast(dev_W());
      float_t *bias_ptr   = mutable_double_cast(dev_Bias());      
      float_t *dw_ptr    = mutable_double_cast(dev_dW());
      float_t *db_ptr    = mutable_double_cast(dev_db());
      float_t *prev_delta_ptr    = mutable_double_cast(dev_prev_delta());
      float_t *curr_delta_ptr    = mutable_double_cast(dev_curr_delta());
      ...
      kernel_->Backward(true, false, NULL, curr_delta_ptr, w_ptr, dw_ptr, bias_ptr, db_ptr, prev_out_ptr, prev_delta_ptr, batch_size);

      //kernel_->Tune(NULL, curr_delta_ptr, w_ptr, dw_ptr, bias_ptr, db_ptr, prev_out_ptr, prev_delta_ptr, batch_size);
      ...
      // Upload data GPU -> CPU
      // TODO(edgar): trigger this only when is needed
      //std::vector<float_t> dW_out(dW[i].size(), 0);
      //dev_dW.Read(queue, dW[i].size(), dW_out);

      //std::vector<float_t> db_out(db[i].size(), 0);
      //dev_db.Read(queue, db[i].size(), db_out);

      std::vector<float_t> prev_delta_out(prev_delta[i].size(), 0);
      dev_prev_delta.Read(queue, prev_delta[i].size(), prev_delta_out);
      ...
      */
      ...
      // copy data to be activated
      //std::copy(std::begin(dW_out), std::end(dW_out), std::begin(dW[i]));
      //std::copy(std::begin(db_out), std::end(db_out), std::begin(db[i]));
      std::copy(std::begin(prev_delta_out), std::end(prev_delta_out), std::begin(prev_delta[i]));
    }

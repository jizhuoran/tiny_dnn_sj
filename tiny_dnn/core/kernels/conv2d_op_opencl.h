...
class Conv2dOpenCLForwardOp : public core::OpKernel {
 public:
  ...
  void compute(core::OpKernelContext &context) override {
    ...
    nn_warn("Got Program");
    __android_log_print(ANDROID_LOG_VERBOSE, "Mytag", "I am really here !!!!");
    ...
    for (size_t i = 0; i < in_data.size(); ++i) {
      ...
      size_t res  = device->device().MaxWorkGroupSize() % 64;
      ...
      auto local  = std::vector<size_t>{8,8,1};
      ...
      /*for (size_t j = 0; j < out.size(); ++j) {
        __android_log_print(ANDROID_LOG_VERBOSE, "Mytag", "%f ", out[j]);
      }*/
      std::cout << std::endl;

      // copy back
      std::copy(std::begin(out), std::end(out),
                std::begin(out_data[i]));
    }
#else

...
class sequential : public nodes {
 public:
  ...
  std::vector<tensor_t> forward(const std::vector<tensor_t> &first) override {
    ...
    double time_before_all = now_ms();
    for (auto l : nodes_) {

      //__android_log_print(ANDROID_LOG_VERBOSE, "My_tag", "layer type: %s", (l->layer_type()).c_str());
      //__android_log_print(ANDROID_LOG_VERBOSE, "My_tag", "input:%u", l->in_size());
      //__android_log_print(ANDROID_LOG_VERBOSE, "My_tag", "output:%u", l->out_size());
      double time_before = now_ms();
     /*std::cout << "layer type:" << nn1[i]->layer_type() << "\n";
      ...
      if(l->layer_type()=="conv") {
        __android_log_print(ANDROID_LOG_VERBOSE, "My_tag", "%lf", (now_ms()- time_before));
      }
    }
    //__android_log_print(ANDROID_LOG_VERBOSE, "My_tag", "!!!!!Time for this network forward is:%lf", (now_ms()- time_before_all));
